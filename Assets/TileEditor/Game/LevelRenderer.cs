...
public class LevelRenderer {
	...
	private static float tUnit = 0.1f;
	...
	public static void BuildMesh(){
		int ti = 0;
		MapData mapdata = (MapData)Resources.Load("Levels/" + MapLoader.currentMap + "/map_data");
		for (int ty=0; ty < mapdata.ySize; ty++)
		{
			for (int tx=0;tx < mapdata.xSize; tx++)
			{
				if(ti < tiler.tileData.tiles.Count())
				{
					...
					if(tid != -1)
					{
						float x1 = Mathf.Round ((Mathf.Repeat (tid, 10))) / 10;
						float y1 = Mathf.Floor ((tid / 100) * 10.0f) / 10.0f;
	...
	public static void GenSquare(int x, int y, Vector2 texture){
		...
		newUV.Add(new Vector2 (texture.x,texture.y + tUnit));
		newUV.Add(new Vector2 (texture.x + tUnit,texture.y + tUnit));
		newUV.Add(new Vector2 (texture.x + tUnit,texture.y));
